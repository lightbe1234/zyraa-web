...
export async function POST(request: Request) {
  try {
    ...
    if (Number(request.headers.get('content-length')) > 4 * 1024 * 1024 + 65536) throw new Error('File too large');
    const file = (await request.formData()).get('file');
    if (!(file instanceof File) || file.size < 12 || file.size > 4 * 1024 * 1024) throw new Error('Invalid file');
    ...
    const { error: auditError } = await db.from('audit_events').insert({ actor_email: 'storefront', action: 'custom_shirt.artwork_uploaded', entity_type: 'artwork', entity_id: path });
    if (auditError) {
      await db.storage.from('custom-artwork').remove([path]);
      throw auditError;
    }
    return Response.json({ path, url: data?.signedUrl }, { status: 201 });
  } catch { return Response.json({ error: 'Upload a PNG, JPG or WebP image under 4 MB.' }, { status: 400 }); }
}
