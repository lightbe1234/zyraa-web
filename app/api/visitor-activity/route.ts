...
export async function POST(request: Request) {
  ...
  if (requireAdmin(request)) return new Response(null, { status: 204 });
  ...
  try {
    ...
    if (!getVisitorActivity().record(visitor, body.action, path, device)) return new Response(null, { status: 429 });
...
export async function GET(request: Request) {
  ...
  try { return Response.json(getVisitorActivity().read(), { headers }); }
