...
export function CustomShirtBanner() {
  ...
  return <a className="zs-banner" href="/customise-your-shirt">
    {image && <img src={image} alt="Custom shirt design service" className="zs-banner-photo" loading="lazy"/>}
...
export function CustomItemDetails({ details }: { details?: CustomDetails }) {
  ...
  return <span className="cs-item-details"><span>{details.fabric} · {details.print}</span>{(front.length||back.length)&&<span>Placement: {[front.length ? `Front — ${front.join(' + ')}` : '', back.length ? `Back — ${back.join(' + ')}` : ''].filter(Boolean).join(' · ')}</span>}{details.instructions && <span>Notes: {details.instructions}</span>}</span>;
}
...
function PositionPicker({title,options,value,onPick}:{title:string;options:CustomOption[];value:string[];onPick:(id:string)=>void}) {
  return <div className="zs-position-picker"><h4>{title}<small>Select all that apply</small></h4><div role="group" aria-label={title}>{options.map(option=>{const selected=value.includes(option.id);return <button type="button" key={option.id} aria-pressed={selected} className={selected?'selected':''} onClick={()=>onPick(option.id)}><span><b>{option.name}</b><small>{option.description}</small></span><strong>+ {money(option.price)}</strong>{selected&&<Check size={15}/>}</button>;})}</div></div>;
}
export function CustomShirtStudio({onAdd}:{onAdd:(product:Product,size:string,colour:string)=>void}) {
  ...
  const [colour,setColour]=useState(''); const [fabric,setFabric]=useState(''); const [print,setPrint]=useState(''); const [frontPositions,setFrontPositions]=useState<string[]>([]); const [backPositions,setBackPositions]=useState<string[]>([]); const [size,setSize]=useState('');
  ...
  const positionsReady=(!needsFront||frontPositions.length>0)&&(!needsBack||backPositions.length>0);
  const complete=[!!colour,!!fabric,!!size&&!!print&&positionsReady,artReady];
  const total=config?.options.filter(o=>[colour,fabric,print,...frontPositions,...backPositions].includes(o.id)).reduce((sum,o)=>sum+o.price,0)||0;
  ...
  async function upload(side:'front'|'back',file?:File){
    ...
    if(!['image/png','image/jpeg','image/webp'].includes(file.type)||file.size>25*1024*1024){setError('Choose a PNG, JPG or WebP under 25 MB.');return;}
    uploadLock.current=true;setUploading(true);setError('');
    try{const form=new FormData();form.set('file',await prepareImage(file));const r=await fetch('/api/custom-shirts/upload',{method:'POST',body:form});const data=await uploadResponse(r);(side==='front'?setFrontArt:setBackArt)({...data,name:file.name});}
  ...
  async function save(e:React.FormEvent){
    ...
    try{
      customQuote(config!.options,{colour,fabric,print,frontPositions,backPositions});
      const r=await fetch('/api/custom-shirts/designs',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({colour,fabric,print,frontPositions,backPositions,size,instructions:notes,frontArtwork:needsFront?frontArt!.path:'',backArtwork:needsBack?backArt!.path:''})});
  ...
  return <main className="zs-studio">
    ...
    <section className="zs-faq"><div><span>BEFORE YOU ORDER</span><h2>A few details.</h2><a href="/pages/contact">Ask us a question <ArrowUpRight size={17}/></a></div><div>{[
      ['Which images can I upload?','Use a clear PNG, JPG or WebP, up to 25 MB before optimisation. A transparent PNG works well if you do not want a background.'],
...
export function CustomShirtAdmin({ viewOrders }: { viewOrders: () => void }) {
  ...
  async function image(file?:File){if(!file||!editing)return;setBusy(true);try{const body=new FormData();body.set('file',await prepareImage(file));const r=await fetch('/api/admin/uploads',{method:'POST',body});const d=await uploadResponse(r);setEditing({...editing,image:d.url});}catch(e){setMessage(e instanceof Error?e.message:'Upload failed.');}finally{setBusy(false);}}
  async function bannerImage(file?:File){if(!file||!config)return;setBusy(true);try{const body=new FormData();body.set('file',await prepareImage(file));const r=await fetch('/api/admin/uploads',{method:'POST',body});const d=await uploadResponse(r);setConfig({...config,settings:{...config.settings,custom_banner_image:d.url}});}catch(e){setMessage(e instanceof Error?e.message:'Upload failed.');}finally{setBusy(false);}}
