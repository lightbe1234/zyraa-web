...
test('custom orders freeze server prices and retain artwork and instructions transactionally', async()=>{
 ...
 try {
  ...
  const item=(await db.query<{unit_price:number;custom_details:{instructions:string;artwork:string;frontArtwork:string;backArtwork:string;frontPosition?:string;backPosition?:string}}>('select unit_price,custom_details from order_items where order_id=$1',[id])).rows[0];
  assert.equal(item.unit_price,229000);
